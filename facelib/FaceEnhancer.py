    ...
    def __init__(self, place_model_on_cpu=False, run_on_cpu=False):
        ...
        class FaceEnhancer (nn.ModelBase):
            ...
            def forward(self, bgr, param, param1):
                x = self.conv1(bgr)
                a = self.dense1(param)
                a = torch.reshape(a, (-1, 64, 1, 1))       # (-1,1,1,64) in NHWC

                b = self.dense2(param1)
                b = torch.reshape(b, (-1, 64, 1, 1))
                ...
                x = F.avg_pool2d(x, 2, 2)                  # tf.nn.avg_pool VALID 2x2
                ...
                x = torch.cat( [nn.resize2d_bilinear(x), e4], dim=nn.conv2d_ch_axis )
                x = F.leaky_relu(self.d4_conv0(x), 0.1)
                x = F.leaky_relu(self.d4_conv1(x), 0.1)

                x = torch.cat( [nn.resize2d_bilinear(x), e3], dim=nn.conv2d_ch_axis )
                x = F.leaky_relu(self.d3_conv0(x), 0.1)
                x = F.leaky_relu(self.d3_conv1(x), 0.1)

                x = torch.cat( [nn.resize2d_bilinear(x), e2], dim=nn.conv2d_ch_axis )
                x = F.leaky_relu(self.d2_conv0(x), 0.1)
                x = F.leaky_relu(self.d2_conv1(x), 0.1)

                x = torch.cat( [nn.resize2d_bilinear(x), e1], dim=nn.conv2d_ch_axis )
                x = F.leaky_relu(self.d1_conv0(x), 0.1)
                x = F.leaky_relu(self.d1_conv1(x), 0.1)

                x = torch.cat( [nn.resize2d_bilinear(x), e0], dim=nn.conv2d_ch_axis )
        ...
        # no-op for inference placement. The cost: mainscripts/Merger.py and
        # mainscripts/FacesetEnhancer.py hardcode/derive place_model_on_cpu on
        # low-VRAM cards expecting the weights to stay off the GPU; under
        # torch they no longer do, i.e. more VRAM used than before -- the
        ...
        # The file ships float16 (72 keys, the only such asset). The layers are
        # float32 (nn.floatx) and load_weights casts -- which is what the TF
        # build did as well, since its variables were float32 too. Building
        # them float16 to "match the file" would change the results.
        self.model.load_weights (model_path)

    def _net_run(self, patch, param, param1):
        """NHWC numpy in, NHWC numpy out."""
        def t(a, shape=None):
            x = torch.as_tensor(np.ascontiguousarray(a)).to(self.device, nn.floatx)
            return x if shape is None else x.reshape(shape)

        x = t(patch).permute(0, 3, 1, 2)
        with torch.no_grad():
            out = self.model(x, t(param, (1, 1)), t(param1, (1, 1)))
    ...
    def enhance (self, inp_img, is_tanh=False, preserve_size=True):
        ...
        while j < j_max:
            ...
            while i < i_max:
                ...
                x = self._net_run( patch_img[None,...], param, param1 )[0]
