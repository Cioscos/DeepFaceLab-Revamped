...
class FANExtractor(object):
    def __init__ (self, landmarks_3D=False, place_model_on_cpu=False):
        ...
        class ConvBlock(nn.ModelBase):
            ...
            def forward(self, input):
                ...
                x = torch.cat([out1, out2, out3], dim=nn.conv2d_ch_axis)

                if self.in_planes != self.out_planes:
                    downsample = self.down_bn1(input)
                    downsample = F.relu(downsample)
        ...
        class HourGlass (nn.ModelBase):
            ...
            def forward(self, input):
                ...
                low1 = F.avg_pool2d(input, 2, 2)       # tf.nn.avg_pool VALID 2x2
                low1 = self.b2 (low1)
        ...
        class FAN (nn.ModelBase):
            ...
            def on_build(self):
                ...
                # Plain lists would be invisible to torch: their layers would
                # never be built, saved or loaded. The ModuleList join rule
                # ('_' between a list parent and its index) reproduces the TF
                # scope names verbatim -- m_0, top_m_0, conv_last_0, bn_end_0,
                # l_0, bl_0, al_0 -- which is what 2DFAN.npy contains.
                m, top_m, conv_last, bn_end, l, bl, al = [], [], [], [], [], [], []
                for i in range(4):
                    m += [ HourGlass(256, 4) ]
                    top_m += [ ConvBlock(256, 256) ]

                    conv_last += [ nn.Conv2D (256, 256, kernel_size=1, strides=1, padding='VALID') ]
                    bn_end += [ nn.BatchNorm2D(256) ]

                    l += [ nn.Conv2D (256, 68, kernel_size=1, strides=1, padding='VALID') ]

                    if i < 4-1:
                        bl += [ nn.Conv2D (256, 256, kernel_size=1, strides=1, padding='VALID') ]
                        al += [ nn.Conv2D (68, 256, kernel_size=1, strides=1, padding='VALID') ]

                self.m         = torch.nn.ModuleList(m)
                self.top_m     = torch.nn.ModuleList(top_m)
                self.conv_last = torch.nn.ModuleList(conv_last)
                self.bn_end    = torch.nn.ModuleList(bn_end)
                self.l         = torch.nn.ModuleList(l)
                self.bl        = torch.nn.ModuleList(bl)
                self.al        = torch.nn.ModuleList(al)
            ...
            def forward(self, x):
                ...
                # The TF version ended with tf.transpose(x, (0,3,1,2)): it
                # converted the NHWC output to the NCHW that
                # get_pts_from_predict reads as (a_ch, a_h, a_w). This network
                # is NCHW already, so the transpose is not preserved -- keeping
                # it would swap channels and rows.
                return outputs[-1]

        # TF wrapped only the model construction+load_weights in the CPU
        # device scope, so place_model_on_cpu kept the weights in host memory
        # while the convolutions still ran on the GPU. A torch module's
        ...
        # ~95MB of resident weights per net onto a sub-4GB card risks turning
        # "slow extraction" into an OOM with no extraction at all. This is a
    ...
    def _net_run(self, img):
        """
        NHWC numpy in, NCHW numpy out (68, 64, 64) per sample.

        NCHW on the way out is not an oversight: get_pts_from_predict reads
        `a_ch, a_h, a_w = a.shape`, so the channel axis has to come first --
        which is exactly what the TF transpose at the end of forward produced.
        """
        ...
        for (left, top, right, bottom) in rects:
            ...
            try:
                ...
                for i in range( len(images) ):
                    predicted += [ self._net_run ( images[i][None,...] )[0] ]
    ...
    def crop(self, image, center, scale, resolution=256.0):
        # np.int / np.float were removed in numpy 2; both were aliases of the
        # builtins, so this is identical in behaviour.
