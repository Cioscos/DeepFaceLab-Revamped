...
def get_transform_mat (image_landmarks, output_size, face_type, scale=1.0):
    ...
    # np.float32 esplicito: l_t nasce float64 perche' `mod` lo e' (float32*float64
    # promuove), e cv2.getAffineTransform di OpenCV 5 esige CV_32F su entrambi gli
    # argomenti -- `src.checkVector(2, CV_32F) == 3` -- dove OpenCV 4, quello del
    # runtime imbarcato in _internal/, accettava anche CV_64F. E' la stessa
    # conversione che gli altri tre chiamanti della funzione fanno gia'
    # (core/mathlib/__init__.py:97, core/imagelib/morph.py:7,
    # mainscripts/Extractor.py:619): qui mancava.
    mat = cv2.getAffineTransform(np.float32(l_t),pts2)
    return mat
