...
# torch va importato PRIMA di PyQt5, e l'ordine e' il fix, non un vezzo.
# Su Windows PyQt5 spedisce in Qt5/bin una propria copia del runtime MSVC
# (msvcp140.dll, vcruntime140.dll) piu' vecchia di quella di sistema, e la
# antepone al PATH quando la si importa. Se Qt5Core.dll la carica per prima,
# c10.dll se la ritrova gia' mappata -- Windows risolve per nome base -- e
# non ci trova cio' che pretende: `import torch` muore con WinError 1114.
# Al contrario torch carica il runtime di sistema, che Qt poi riusa senza
# lamentarsi. Non e' teoria: questo modulo lo reimportano da zero i processi
# figli di LoaderQSubprocessor, dove nessuno ha ancora caricato torch, ed e'
# li' che l'editor si rompeva mentre il processo padre -- che passa da
# main.py, quindi da core.leras.nn, quindi da torch -- restava sano.
# L'import e' inutilizzato qui dentro di proposito: serve l'effetto, non il
# nome. Non rimuoverlo e non spostarlo sotto gli import di PyQt5.
import torch
...
class QCanvasOperator(QWidget):
    ...
    def img_to_cli_rect(self, rect):
        ...
        # quindi il comportamento e' identico.
