...
def flatten(x):
    # torch.flatten keeps the batch axis symbolic under tracing; the previous
    # reshape(-1, int(np.prod(x.shape[1:]))) froze the product through a
    # Python int and every ONNX export printed a TracerWarning for it. Same
    # values, same shapes: only the trace changes.
...
def random_binomial(shape, p=0.0, dtype=None, seed=None, device=None):
    ...
    # `device` places the draw itself, generator included, not just the
    # result: the lr_dropout mask is consumed on the weight's device, and
    # sampling it there avoids a host->device copy per parameter per step
    # (the TF build placed this op on GPU too, unless lr_dropout_on_cpu).
    # None keeps the pre-existing host-side draw and its sequence.
    gen = None
    if seed is not None:
        gen = torch.Generator(device='cpu' if device is None else device)
    ...
    u = torch.rand(shape, generator=gen, dtype=torch.float32, device=device)
    return torch.where(u < p, torch.ones(shape, dtype=dtype, device=device),
