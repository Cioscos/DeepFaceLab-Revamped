...
class AdaBelief(nn.OptimizerBase):
    def __init__(self, lr=0.001, beta_1=0.9, beta_2=0.999, lr_dropout=1.0, lr_cos=0, clipnorm=0.0, name=None, **kwargs):
        ...
        # still resampled every sess.run. nn.random_binomial (core/leras/
        # ops/__init__.py) has no device parameter and always samples with a
        # CPU generator, so this flag has no numerical effect here -- but it
        # is also why every masked step() now does a host->device copy of
        # the mask per parameter when v is on GPU, where the TF build could
        # avoid that copy by sampling directly on the GPU. Not a correctness
        # concern, just a per-iteration cost worth knowing about.
    ...
    def step(self, grads_vars):
        ...
        with torch.no_grad():
            ...
            for g, v in grads_vars:
                ...
                if self.lr_cos != 0:
                    lr = lr * (torch.cos(self.iters.to(g_c.dtype) *
