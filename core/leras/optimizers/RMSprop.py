...
class RMSprop(nn.OptimizerBase):
    def __init__(self, lr=0.001, rho=0.9, lr_dropout=1.0, lr_cos=0, clipnorm=0.0, name=None, **kwargs):
        ...
        # See AdaBelief.py: no numerical effect given random_binomial's
        # CPU-only generator, but the TF build sampled on GPU when this was
        # False, so a masked step() here now pays a host->device copy per
        # parameter that the TF build didn't.
        ...
        with torch.no_grad():
            ...
            for g, v in grads_vars:
                ...
                if self.lr_dropout != 1.0:
                    # Resampled every step -- see AdaBelief.step.
