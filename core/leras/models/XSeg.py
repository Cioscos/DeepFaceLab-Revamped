...
class XSeg(nn.ModelBase):
    ...
    def forward(self, inp, pretrain=False):
        ...
        x = self.uconv53(torch.cat([x,x5],axis=nn.conv2d_ch_axis))
        x = self.uconv52(x)
        x = self.uconv51(x)
        ...
        x = self.uconv43(torch.cat([x,x4],axis=nn.conv2d_ch_axis))
        x = self.uconv42(x)
        x = self.uconv41(x)
        ...
        x = self.uconv33(torch.cat([x,x3],axis=nn.conv2d_ch_axis))
        x = self.uconv32(x)
        x = self.uconv31(x)
        ...
        x = self.uconv22(torch.cat([x,x2],axis=nn.conv2d_ch_axis))
        x = self.uconv21(x)

        x = self.up1(x)
        if pretrain:
            x1 = torch.zeros_like(x1)
        x = self.uconv12(torch.cat([x,x1],axis=nn.conv2d_ch_axis))
        x = self.uconv11(x)

        x = self.up0(x)
        if pretrain:
            x0 = torch.zeros_like(x0)
        x = self.uconv02(torch.cat([x,x0],axis=nn.conv2d_ch_axis))
        x = self.uconv01(x)
