...
class PatchDiscriminator(nn.ModelBase):
    def on_build(self, patch_size, in_ch, base_ch=None, conv_kernel_initializer=None):
        ...
        for i, (kernel_size, strides) in enumerate(kernels_strides):
            ...
            convs.append ( nn.Conv2D( prev_ch, cur_ch, kernel_size=kernel_size, strides=strides, padding='SAME', kernel_initializer=conv_kernel_initializer) )
            prev_ch = cur_ch
...
class UNetPatchDiscriminator(nn.ModelBase):
    ...
    def on_build(self, patch_size, in_ch, base_ch = 16, use_fp16 = False):
        ...
        for i, (kernel_size, strides) in enumerate(layers):
            convs.append ( nn.Conv2D( level_chs[i-1], level_chs[i], kernel_size=kernel_size, strides=strides, padding='SAME', dtype=conv_dtype) )

            upconvs.insert (0, nn.Conv2DTranspose( level_chs[i]*(2 if i != len(layers)-1 else 1), level_chs[i-1], kernel_size=kernel_size, strides=strides, padding='SAME', dtype=conv_dtype) )
        self.convs = torch.nn.ModuleList(convs)
    ...
    def forward(self, x):
        ...
        for i, (upconv, enc) in enumerate(zip(self.upconvs, encs)):
            ...
            x = torch.cat( [enc, x], axis=nn.conv2d_ch_axis)
