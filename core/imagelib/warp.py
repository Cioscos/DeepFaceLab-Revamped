...
def gen_warp_params (w, flip=False, rotation_range=[-10,10], scale_range=[-0.5, 0.5], tx_range=[-0.05, 0.05], ty_range=[-0.05, 0.05], rnd_state=None, warp_rnd_state=None  ):
    ...
    #random transform
    # int(): w arriva da self.options['resolution'], che e' un np.int64 (np.clip).
    # Il cv2 5.0.0 rifiuta i tipi numpy nel 'center' di getRotationMatrix2D
    # (verificato); il 4.1.0.25 del pacchetto li accettava, secondo la traccia
    # d'errore che questa riga produceva. Cast, non cambio di semantica.
