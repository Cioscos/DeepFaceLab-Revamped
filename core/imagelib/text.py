...
def get_text_image( shape, text, color=(1,1,1), border=0.2, font=None):
    ...
    try:
        ...
        # quindi il comportamento e' identico. Qui l'except nudo di sotto
        # inghiottiva l'AttributeError e la scritta spariva in silenzio.
...
def get_draw_text_lines ( image, rect, text_lines, color=(1,1,1), border=0.2, font=None):
    # np.float e' stato rimosso in numpy 2; era l'alias del builtin, che
    # np.zeros risolve a float64 -- lo stesso dtype di default. Stessa classe di
    # difetto del np.int corretto sopra, in questo stesso modulo: fuori dal
    # percorso del training (solo l'extractor manuale passa di qui) e innocuo
    # sullo stack distribuito, ma rumoroso sotto numpy 2.
