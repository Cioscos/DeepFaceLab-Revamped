...
class ModelBase(object):
    ...
    def update_sample_for_preview(self, choose_preview_history=False, force_new=False):
        if self.sample_for_preview is None or choose_preview_history or force_new:
            if choose_preview_history and io.is_support_windows():
                if sys.platform != 'win32' and threading.current_thread() is not threading.main_thread():
                    # HighGUI on this platform is Qt/GTK, and both require GUI
                    # calls on the thread that owns the GUI. This method runs
                    # on the trainer thread: opening the chooser here leaves
                    # the training preview window (created later by the main
                    # thread) permanently blank. Same outcome as answering 'n'.
                    io.log_info("Choosing the image for the preview history is only supported on Windows here: on this platform the GUI backend requires window calls on the main thread. Using random preview samples instead.")
        ...
        try:
            ...
            io.log_info( f"Warning: could not generate the history previews ({e}), regenerating the preview samples." )
            self.sample_for_preview = self.generate_next_samples()
