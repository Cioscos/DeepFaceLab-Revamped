...
def sort_by_absdiff(input_path):
    ...
    # data_format="NHWC" e' caduto con la sessione TF: era li' solo per
    # ottenerla, e il core/leras portato solleva su tutto cio' che non e' NCHW.
    # La scelta del device resta: absdiff_batch legge nn.device.
    ...
    while j < image_paths_len:
        ...
        while i < image_paths_len:
            if i >= j:
                ...
                result = absdiff_batch (i_images, j_images)
                db[j:j+j_images_len,i:i+i_images_len] = result
                io.progress_bar_inc(1)
